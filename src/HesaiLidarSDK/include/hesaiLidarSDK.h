...
public:
	...
	HesaiLidarSDK(
		...
		boost::function<void(std::list<PandarPacket> pkts, boost::shared_ptr<PPointCloud> pcloudp, double timestamp)> lidarCallback, 				// lidar的callback函数
	...
	HesaiLidarSDK(
		boost::function<void(std::list<PandarPacket> pkts, boost::shared_ptr<PPointCloud> pcloudp, double timestamp)> lidarCallback);
